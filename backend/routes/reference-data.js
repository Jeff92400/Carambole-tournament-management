...
router.get('/poule-config-preview', authenticateToken, async (req, res) => {
  try {
    ...
    console.error('Error computing poule config preview:', err);
...
router.get('/scoring-fields', authenticateToken, (req, res) => {
  const fields = [
    ...
    { code: 'POSITION', label: 'Position finale', type: 'integer', scope: 'player',
      description: 'Classement final du joueur dans le tournoi (global)' },
    { code: 'POULE_RANK', label: 'Rang dans la poule', type: 'integer', scope: 'player',
      description: 'Classement du joueur dans sa poule (1 = 1er de la poule, calculé depuis les convocations)' },
